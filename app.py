...
# ==================== 讀取獨立圖檔並轉換為 Base64（支援 10 個自定義體態圖檔） ====================
@st.cache_data
def get_character_avatar_base64(gender, tier_idx):
  # 依據 BMI 等級對應到你上傳的 5 種身態稱呼
  tier_mapping = {0: "很瘦", 1: "瘦", 2: "正", 3: "微胖", 4: "很胖"}
  status_key = tier_mapping.get(tier_idx, "正")

  image_filename = f"{gender}-{status_key}.png"
...
TAIWAN_FOOD_DB = [
    ...
    {
        "name": "飯糰 (Taiwanese Rice Ball, 1顆)",
        "cal": 420.0,
        "pro": 12.0,
        "carb": 58.0,
        "fat": 15.0,
    ...
    {
        "name": "雞排 (Fried Chicken Cutlet, 1份)",
        "cal": 650.0,
        "pro": 35.0,
        "carb": 40.0,
        "fat": 42.0,
    ...
    {
        "name": "鍋貼 (Pan-fried Dumplings, 8顆)",
        "cal": 600.0,
        "pro": 18.0,
        "carb": 65.0,
        "fat": 30.0,
    ...
    {
        "name": "鹹酥雞 (Salt Crispy Chicken, 1份)",
        "cal": 550.0,
        "pro": 25.0,
        "carb": 30.0,
        "fat": 35.0,
    ...
    {
        "name": "水煮雞胸肉 (Chicken Breast, 100g)",
        "cal": 165.0,
        "pro": 31.0,
        "carb": 0.0,
        "fat": 3.6,
...
with tab2:
  ...
  if st.session_state.history:
    ...
    filter_meal = st.selectbox(
        "篩選想查看的餐別", ["全部"] + list(df["meal"].unique())
    )
    df_filtered = (
        df[df["meal"] == filter_meal] if filter_meal != "全部" else df
    )
    ...
    chart = (
        ...
        .mark_bar(
            color="#ff4b4b", cornerRadiusTopLeft=4, cornerRadiusTopRight=4
        )
        .encode(
            x=alt.X(
                "food:N",
                sort=None,
                title="食物名稱",
                axis=alt.Axis(labelAngle=-25, labelLimit=250),
            ),
            y=alt.Y(
                "cal:Q",
                title="熱量 (大卡)",
                axis=alt.Axis(titleAngle=0, titleAnchor="end", titleY=-10),
            ),
    ...
    st.info(
        f"💡 系統總結：今日累計攝取 **{total_cal_today} 大卡** (目標 TDEE："
        f" {tdee:.0f} 大卡)"
    )

    if total_cal_today > tdee:
      st.error(
          "⚠️ 【隔天超標智能提醒】注意！昨天熱量超標囉！建議今天早餐改吃水煮蛋與無糖豆漿，午餐主食減半，將熱量拉回平衡！"
      )
    else:
      st.success(
          f"✨ 【完美狀態】『{char_name}』保持得非常棒！請繼續維持這個節奏！"
      )
  else:
...
with tab3:
  ...
  st.info(
      f"💡 根據您的身高 (**{height} cm**) 與體重 (**{weight} kg**) 計算，今日建議飲水量為："
  ...
  if st.session_state.water < recommended_water:
    st.warning("⚠️ 警告：角色出現『缺水』，代謝速度下降中，請趕快多喝水！")
  else:
    st.success("🌟 狀態加成：水分充足，獲得『水潤新陳代謝』！")
...
with tab4:
  ...
  st.info(
      "💡 請分別輸入早餐、午餐、晚餐吃了什麼，系統將自動計算全天總熱量與營養素是否超過，並給予建議與模擬報告！"
  )

  st.markdown("---")
  st.markdown("#### 🍳 早餐：吃了甚麼")
  bf_name, bf_cal, bf_pro, bf_carb, bf_fat = render_food_selector_section(
      "tab4_breakfast"
  )

  st.markdown("---")
  st.markdown("#### 🍱 午餐：吃了甚麼")
  lu_name, lu_cal, lu_pro, lu_carb, lu_fat = render_food_selector_section(
      "tab4_lunch"
  )

  st.markdown("---")
  st.markdown("#### 🍲 晚餐：吃了甚麼")
  di_name, di_cal, di_pro, di_carb, di_fat = render_food_selector_section(
      "tab4_dinner"
  )
  ...
  if st.button("🚀 啟動模擬分析與建議", type="primary"):
    ...
    if projected_remaining >= 150:
      st.success(
          "🌟 你的熱量扣打相當充裕，今天這樣吃非常完美，完全符合目標體重進度，外觀保持極佳！"
      )
    elif projected_remaining >= 0:
      st.warning(
          "⚠️ 今天的熱量剛好達到 TDEE 邊界。建議稍微控制宵夜或增加一點日常活動量，以維持完美體態。"
      )
    else:
      st.error(
          "🚨 今天這樣吃將導致熱量超標約"
          f" **{abs(projected_remaining):.0f} kcal**！建議減少其中一餐的份量或增加有氧運動來平衡！"
      )
